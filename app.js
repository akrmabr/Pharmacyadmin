(() => {
  ...
  const DEFAULT_ADMIN_EMAIL = 'akrmabrahymk@gmail.com';
  ...
  $('licenseType').addEventListener('change',e=>{$('dateField').hidden=e.target.value==='LIFETIME';});
  $('createForm').addEventListener('submit',async event=>{event.preventDefault();const f=new FormData(event.currentTarget),type=f.get('license_type');const payload={operation:'create_license',pharmacy_name:String(f.get('pharmacy_name')||'').trim(),owner_name:String(f.get('owner_name')||'').trim(),owner_email:String(f.get('owner_email')||'').trim(),license_type:type};if(type!=='LIFETIME')payload.expires_at=String(f.get('expires_at')||'');try{showMessage('جارٍ إصدار الترخيص…');const result=await call('owner-control',payload);const lic=result.license||result,key=lic.license_key||result.key;const box=$('createdResult');box.hidden=false;box.innerHTML=`<strong>تم إصدار الترخيص بنجاح</strong><div dir="ltr">${display(key)}</div><button type="button" class="ghost copy-key">نسخ مفتاح الترخيص</button><pre>${JSON.stringify(result,null,2)}</pre>`;box.querySelector('.copy-key').addEventListener('click',()=>navigator.clipboard?.writeText(String(key||'')));showMessage('تم إصدار الترخيص وتحديث اللوحة.','success');event.currentTarget.reset();$('dateField').hidden=false;await refresh();}catch(e){showMessage(e.message||'فشل إصدار الترخيص.','error');}});
